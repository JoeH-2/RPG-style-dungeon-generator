...
#ADD INTO ARMOUR
def submit(Name,Rarity,Type,Armour):
    conn = sqlite3.connect('Armour.db')
    c = conn.cursor()

    c.execute("INSERT INTO Armours VALUES (?,?,?,?)",(Name,Rarity,Type,Armour))
    c.execute("SELECT * FROM Armours")
    items = c.fetchall()
    print('''''')
    for item in items:
...
def delete(Name,Rarity,Type,Armour):
    conn = sqlite3.connect('Armour.db')
    c = conn.cursor()
    c.execute("DELETE from Armours WHERE Name=? AND Rarity=? AND Type=? AND Armour=?", (Name,Rarity,Type,Armour))
    c.execute("SELECT * FROM Armours")
    items = c.fetchall()
    print('''''')
    for item in items:
...
Attack_Label = Label(root, text="Armour")
